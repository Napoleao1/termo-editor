...
def resource_path(relative_pimport os
import sys
...
class WordFormApp(QWidget):
    ...
    def initUI(self):
        ...
        labels = [
            "Nome", "CPF", "Endereço", "CEP", "Celular", "E-mail", "Equipamento",
            "Patrimônio", "Número de Série", "Observações", "Assinatura do Colaborador", "Responsável Técnico"
        ]
        ...
        for label in labels:
            ...
            if label == "Responsável Técnico":
                ...
                combo = QComboBox()
                combo.addItems(["", "Retirada", "Devolução", "Desligamento"])
                self.fields[label] = combo
                layout.addWidget(combo)
            else:
                self.fields[label] = QLineEdit()
        ...
        layout.addWidget(QLabel("Equipamentos Adicionais:"))
        equipamentos = [
            "01 Mouse sem fio", "Teclado com fio", "Headset Logitech", "Headset JBL",
            "Suporte Notebook", "Mouse pad", "Adaptador Hub USB-C",
            "Carregador/fonte Apple", "Carregador/fonte Dell 65W", "Carregador/fonte Dell 45W"
        ]
        equip_box = QGroupBox()
    ...
    def salvar_dados(self):
        ...
        for key, field in self.fields.items():
            if isinstance(field, QLineEdit):
                ...
                dados[key] = field.date().toString(Qt.DateFormat.ISODate)
        dados["Equipamentos Adicionais"] = [cb.text() for cb in self.equip_checkboxes if cb.isChecked()]
        path, _ = QFileDialog.getSaveFileName(self, "Salvar Dados", "", "JSON (*.json)")
        if path:
            with open(path, "w", encoding="utf-8") as f:
                ...
                for key, field in self.fields.items():
                    if key in dados:
                        if isinstance(field, QLineEdit):
                            ...
                            field.setDate(QDate.fromString(dados[key], Qt.DateFormat.ISODate))
                for cb in self.equip_checkboxes:
                    cb.setChecked(cb.text() in dados.get("Equipamentos Adicionais", []))
            QMessageBox.information(self, "Carregado", "Dados carregados com sucesso.")
    ...
    def generate_doc(self):
        # Aqui vai sua lógica de gerar o documento .docx, como já tinha antes
        QMessageBox.information(self, "Aviso", "Documento gerado com sucesso (lógica a ser adicionada).")
